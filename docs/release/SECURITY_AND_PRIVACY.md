# QWSG Community Security and Privacy
...
Current native update discovery authenticates the fixed official release index;
artifact downloads are restricted to its approved public Forgejo source. Requests
are credential-free; network-level source visibility still applies. Downloads enter private user-owned staging
...
Task 082 binds artifact digest/size/provenance and the exact source/target
migration declaration to the bundled publisher key. The declaration must match
a locally implemented capability; signed metadata cannot provide migration code.
Offline archives require the full signed index companion as well as the checksum.
The updater selects only an authenticated compatible migration,
...
source for explicit `update check`. Task 079 lets Guardian perform the
identical fixed anonymous request at most once per 24-hour interval when due.
...
only for configured Alert, lifecycle or update-availability delivery, or an explicit test. TLS verification cannot be
