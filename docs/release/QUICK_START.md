# QWSG 1.3.1 Quick Start

QWSG 1.3.1 is a local, non-root Linux Server Guardian. Verify the downloaded archive checksum, unpack it, then install the fixed `/usr/local` artifact set:

```sh
sha256sum -c qwsg-1.3.1-linux-amd64.tar.gz.sha256
tar -xzf qwsg-1.3.1-linux-amd64.tar.gz
cd qwsg-1.3.1-linux-amd64
