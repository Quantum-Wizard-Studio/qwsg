# QWSG 1.1.0-rc.3 External Clean-Host Acceptance Protocol

## Authority and execution boundary

This protocol applies only to one private RC.3 archive built reproducibly from
...
checkpoint at a time. Each phase requires its explicit Owner gate: A2 source
integration; B/C/D private construction and proof; E private transfer; F
external execution; and G evidence integration/verdict. A PASS or READY does
...
- **Gate A2:** integrate only the reviewed RC.3 acceptance-source allowlist.
- **Gate B/C/D:** export the authorized clean commit twice into independent
  mode-0700 roots; derive `SOURCE_DATE_EPOCH` from its commit timestamp; embed
  the full commit; prove binary, manifest, archive and sidecar byte identity;
  verify static Linux amd64 identity, safe layout, documentation, LICENSE and
  exclusions.
- **Gate E:** transfer exactly the archive and sidecar by Owner-approved strict
...
- **Gate F:** begin the following external protocol only after explicit Owner
...
- **Purpose:** establish the RC.3 transfer identity before touching bytes.
- **Action:** list only the approved destination directory entries and file
  types using the exact bounded command supplied at Gate F.
- **Expected evidence:** exactly the RC.3 archive and `.sha256` sidecar, both regular
  files and neither a symlink.
- **PASS:** count, names and types match the Gate E record.
...
- **Action:** run `sha256sum -c qwsg-1.1.0-rc.3-linux-amd64.tar.gz.sha256` in
...
## Checkpoint 05 — LICENSE, documentation and RC.3 identity

- **Purpose:** verify required legal/operator material and candidate identity.
- **Action:** inspect root `LICENSE`, `README.md`, `INSTALL.md`, RC.3 release
  notes and binary version/source metadata using their documented commands.
- **Expected evidence:** byte-correct LICENSE; usable root docs; RC.3 notes; binary
  version `1.1.0-rc.3`; exact Gate D source commit/build time.
- **PASS:** the expected evidence is observed exactly.
...
- **Purpose:** independently decide whether RC.3 corrects the blocker.
...
- **Action:** after explicit Owner confirmation, reinstall the same verified
  RC.3 candidate, run `qwsg setup` to detect/resume preserved state, explicitly
...
findings or missing mandatory evidence. Gate G may integrate privacy-safe
