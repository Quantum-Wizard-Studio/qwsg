# QWSG 1.1.0-rc.1 External Clean-Host Acceptance Protocol
...
inside Task 049. Engineering knowledge must not silently fill a product or
documentation gap.
...
- **Action:** Place only `qwsg-1.1.0-rc.1-linux-amd64.tar.gz` and its `.sha256`
  sidecar in a new private directory, then run:

  ```sh
  umask 077
  mkdir qwsg-1.1.0-rc.1-acceptance
  mv qwsg-1.1.0-rc.1-linux-amd64.tar.gz qwsg-1.1.0-rc.1-linux-amd64.tar.gz.sha256 qwsg-1.1.0-rc.1-acceptance/
  cd qwsg-1.1.0-rc.1-acceptance
  ls -l
  ```
...
- **Action:** From the private directory run:
  ...
  tar -tzf qwsg-1.1.0-rc.1-linux-amd64.tar.gz
  ```

- **Expected evidence:** Sidecar `OK`; every member is below the single
  `qwsg-1.1.0-rc.1-linux-amd64/` root with no absolute or parent path.
...
- **Expected evidence:** README and INSTALL cross-reference each other, expose
  the supported platform and complete next journey, and the manifest passes.
...
- **Expected evidence:** Version is `1.1.0-rc.1` with the approved full source
