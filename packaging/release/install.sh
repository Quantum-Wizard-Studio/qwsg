...
for file in "$root"/docs/*.md "$root/README.md" "$root/INSTALL.md" "$root/LICENSE" "$root/CHANGELOG.md" "$root/qwsg-config.json" "$root/qwsg-release-trust.json" "$root/RELEASE.json"; do install_one "$file" "$destdir/usr/local/share/doc/qwsg/$(basename "$file")" 0644; done
