...
func TestCompatibilityMigration(t *testing.T) {
	for _, path := range [][2]string{{"1.1.0", "1.2.0-rc.1"}, {"1.1.0", "1.2.0-rc.2"}, {"1.2.0-rc.1", "1.2.0-rc.2"}, {"1.2.0-rc.2", "1.2.0-rc.6"}} {
...
func TestRC2ToRC6SelectsCanonicalPath(t *testing.T) {
	m, err := PlanMigration("1.2.0-rc.2", "1.2.0-rc.6")
	if err != nil || m.ID != "compat-1.2.0-rc.2-to-1.2.0-rc.6" || !m.ReplaceGuardianUnit {
		t.Fatalf("unexpected path: %+v %v", m, err)
	}
}

func TestUnknownMigrationFailsClosed(t *testing.T) {
	for _, path := range [][2]string{{"1.1.0", "1.3.0"}, {"1.2.0-rc.2", "1.2.0-rc.5"}, {"unknown", "1.2.0-rc.6"}, {"1.2.0-rc.6", "1.2.0-rc.6"}} {
