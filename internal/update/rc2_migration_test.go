...
func TestRC2FixtureMigratesToFinalAndRollsBackExactly(t *testing.T) {
	...
	migration, err := PlanMigration(fixture.Version, "1.2.0")
	...
	files := map[string]string{
		...
		"RELEASE.json":       `{"Schema":"qwsg.release/1","Version":"1.2.0","Commit":"1111111111111111111111111111111111111111","Built":"2026-08-29T00:00:00Z","Platform":"linux-amd64"}`,
	...
	if err != nil || !tx.Complete || tx.FromVersion != fixture.Version || tx.ToVersion != "1.2.0" {
	...
	if got, _ := os.ReadFile(filepath.Join(dest, "usr/local/bin/qwsg")); string(got) != "QWSG 1.2.0\n" {
		t.Fatalf("resulting version is not final 1.2.0: %q", got)
	}
