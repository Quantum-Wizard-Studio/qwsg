...
func AssessInstall(ctx context.Context, host Host, now time.Time) Report {
	...
	findings = append(findings, systemdVersion(ctx, host), commandFinding(ctx, host, "systemd.user_manager", "systemd_user", true))
	findings = append(findings, Finding{RequirementID: "filesystem.local_semantics", Classification: UnknownVerification, EvidenceToken: "filesystem_manual_verification_required"})
	SortFindings(findings)
	domain := Summarize(findings)
	report := Report{SchemaName: SchemaName, SchemaVersion: SchemaVersion, ModelVersion: ModelVersion, RegistryVersion: RegistryVersion, AssessedAt: now.UTC(), Phase: "install", Platform: platform, Findings: findings, Domains: []DomainSummary{{Domain: "environment_dependencies", State: domain}, {Domain: "installation", State: domain}}, NextActions: nextInstall(domain)}
	return report
}
...
func nextInstall(state SummaryState) []string {
