...
var english = map[MessageID]string{
	"plan.mode": "Mode: %s", "plan.mode.fresh": "fresh installation", "plan.mode.existing": "verified existing installation / setup", "plan.mode.upgrade": "supported upgrade source", "plan.mode.legacy": "legacy installation", "plan.mode.unknown": "unknown unverified installation", "plan.mode.inconsistent": "inconsistent or incomplete installation",
	...
	"install.upgrade": "A supported older QWSG package is installed. Guided installation will not overwrite it; use the explicit `qwsg update` workflow.", "install.legacy": "A legacy QWSG binary exists without verified package evidence. Back it up and resolve it explicitly before retrying; QWSG will not overwrite or treat it as installed.", "install.unknown": "Unverified QWSG artifacts exist. Review and resolve them explicitly before retrying; QWSG will not overwrite them.", "install.inconsistent": "The installed QWSG package evidence is incomplete, inconsistent, or unsafe. Review and repair or remove it explicitly before retrying; QWSG will not overwrite it.",
	...
	"readiness.ready": "Readiness: ready. Guardian produced the required operational evidence.", "readiness.partial": "Readiness: incomplete. Installation data is preserved; follow `qwsg readiness` and resume setup after correcting the reported requirement.", "readiness.timeout": "Fresh Guardian evidence was not available before the bounded timeout. Installation data is preserved. Run `qwsg readiness`, then resume with `qwsg setup`.",
...
var hungarian = map[MessageID]string{
	"plan.mode": "Mód: %s", "plan.mode.fresh": "új telepítés", "plan.mode.existing": "ellenőrzött meglévő telepítés / beállítás", "plan.mode.upgrade": "támogatott frissítési forrás", "plan.mode.legacy": "örökölt telepítés", "plan.mode.unknown": "ismeretlen, nem ellenőrzött telepítés", "plan.mode.inconsistent": "ellentmondásos vagy hiányos telepítés", "plan.package": "Csomagfájlok: /usr/local/bin/qwsg, /usr/local/lib/qwsg, /usr/local/share/doc/qwsg", "plan.data": "Kezelői adatok: privát konfiguráció és állapot a felhasználói fiók alatt", "plan.service": "Szolgáltatás: qwsg-guardian.service systemd felhasználói egység",
	"install.existing": "Az ellenőrzött QWSG-csomag már telepítve van; a csomagmódosítás kimarad.", "install.failure": "A csomagtelepítés sikertelen. A QWSG nem állítja, hogy a csomagmódosítás sikerült; újrapróbálás előtt tekintse át a diagnosztikát.", "install.upgrade": "Egy támogatott régebbi QWSG-csomag van telepítve. A telepítővarázsló nem írja felül; használja az explicit `qwsg update` folyamatot.", "install.legacy": "Egy örökölt QWSG-bináris létezik ellenőrzött csomagbizonyíték nélkül. Mentse el és rendezze explicit módon az újrapróbálás előtt; a QWSG nem írja felül és nem tekinti telepítettnek.", "install.unknown": "Nem ellenőrzött QWSG-összetevők léteznek. Tekintse át és rendezze őket explicit módon az újrapróbálás előtt; a QWSG nem írja felül őket.", "install.inconsistent": "A telepített QWSG-csomag bizonyítéka hiányos, ellentmondásos vagy nem biztonságos. Tekintse át, javítsa vagy távolítsa el explicit módon az újrapróbálás előtt; a QWSG nem írja felül.", "configuration.done": "A privát kezelői konfiguráció biztonságosan létrejött.", "configuration.failure": "A konfiguráció nem hozható létre. Javítsa a konfigurációs útvonalat vagy értéket, majd próbálja újra.",
	...
	"readiness.timeout": "A friss Guardian-bizonyíték nem érkezett meg a korlátozott várakozási időn belül. A telepítési adatok megmaradtak. Futtassa a `qwsg readiness` parancsot, majd folytassa a `qwsg setup` paranccsal.",
...
var german = map[MessageID]string{
	"plan.mode": "Modus: %s", "plan.mode.fresh": "Neuinstallation", "plan.mode.existing": "verifizierte vorhandene Installation / Einrichtung", "plan.mode.upgrade": "unterstützte Aktualisierungsquelle", "plan.mode.legacy": "Legacy-Installation", "plan.mode.unknown": "unbekannte nicht verifizierte Installation", "plan.mode.inconsistent": "widersprüchliche oder unvollständige Installation", "plan.package": "Paketdateien: /usr/local/bin/qwsg, /usr/local/lib/qwsg, /usr/local/share/doc/qwsg", "plan.data": "Bedienerdaten: private Konfiguration und Zustände im Benutzerkonto", "plan.service": "Dienst: systemd-Benutzereinheit qwsg-guardian.service",
	"install.existing": "Das verifizierte QWSG-Paket ist bereits installiert; die Paketänderung wird übersprungen.", "install.failure": "Paketinstallation fehlgeschlagen. QWSG behauptet keine erfolgreiche Paketänderung; prüfen Sie vor einem neuen Versuch die Diagnose.", "install.upgrade": "Ein unterstütztes älteres QWSG-Paket ist installiert. Der Installationsassistent überschreibt es nicht; verwenden Sie den expliziten Ablauf `qwsg update`.", "install.legacy": "Ein altes QWSG-Programm ist ohne verifizierten Paketnachweis vorhanden. Sichern und klären Sie es ausdrücklich vor einem neuen Versuch; QWSG überschreibt es nicht und betrachtet es nicht als installiert.", "install.unknown": "Nicht verifizierte QWSG-Artefakte sind vorhanden. Prüfen und klären Sie sie ausdrücklich vor einem neuen Versuch; QWSG überschreibt sie nicht.", "install.inconsistent": "Der installierte QWSG-Paketnachweis ist unvollständig, widersprüchlich oder unsicher. Prüfen, reparieren oder entfernen Sie ihn ausdrücklich vor einem neuen Versuch; QWSG überschreibt ihn nicht.", "configuration.done": "Die private Bedienerkonfiguration wurde sicher initialisiert.", "configuration.failure": "Die Konfiguration konnte nicht initialisiert werden. Korrigieren Sie Pfad oder Wert und versuchen Sie es erneut.",
	...
	"readiness.timeout": "Vor Ablauf der begrenzten Wartezeit war kein frischer Guardian-Nachweis verfügbar. Die Installationsdaten bleiben erhalten. Führen Sie `qwsg readiness` aus und setzen Sie dann mit `qwsg setup` fort.",
