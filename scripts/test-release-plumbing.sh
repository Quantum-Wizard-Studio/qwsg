...
test ! -e "$repo/dist/qwsg-1.3.0-linux-amd64.tar.gz"
test "$(QWSG_RELEASE_VALIDATE_ONLY=1 "$repo/scripts/build-release.sh")" = \
  'release build: identity 1.3.0 is valid'
...
mkdir "$work/dist"
touch "$work/dist/qwsg-1.3.0-linux-amd64.tar.gz"
...
grep -F 'qwsg-1.3.0-linux-amd64.tar.gz' "$repo/docs/installation/INSTALL.md" >/dev/null
...
printf '%s\n' 'PASS: QWSG 1.3.0 release plumbing'
