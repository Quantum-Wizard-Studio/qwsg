...
test "$(tr -d '\r\n' < "$repo/VERSION")" = 1.4.0
test -f "$repo/docs/release/RELEASE_NOTES_1.4.0.md"
test -f "$repo/docs/release/RELEASE_NOTES_1.3.1.md"
test -f "$repo/docs/release/RELEASE_NOTES_1.3.0.md"
test -f "$repo/docs/release/RELEASE_NOTES_1.2.0.md"
test -f "$repo/docs/release/ACCEPTANCE_1.2.0.md"
test -f "$repo/docs/release/RELEASE_NOTES_1.2.0-rc.7.md"
...
test ! -e "$repo/dist/qwsg-1.4.0-linux-amd64.tar.gz"
test "$(QWSG_RELEASE_VALIDATE_ONLY=1 "$repo/scripts/build-release.sh")" = \
  'release build: identity 1.4.0 is valid'
...
mkdir "$work/dist"
touch "$work/dist/qwsg-1.4.0-linux-amd64.tar.gz"
...
grep -F 'qwsg-1.3.1-linux-amd64.tar.gz' "$repo/docs/installation/INSTALL.md" >/dev/null
...
printf '%s\n' 'PASS: QWSG 1.4.0 candidate release plumbing'
