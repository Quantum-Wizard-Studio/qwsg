...
test ! -e "$repo/dist/qwsg-1.3.1-linux-amd64.tar.gz"
test "$(QWSG_RELEASE_VALIDATE_ONLY=1 "$repo/scripts/build-release.sh")" = \
  'release build: identity 1.3.1 is valid'
...
mkdir "$work/dist"
touch "$work/dist/qwsg-1.3.1-linux-amd64.tar.gz"
...
printf '%s\n' 'PASS: QWSG 1.3.1 release plumbing'
