...
2. Run `bin/job --check` from the verified repository. Classify an unexpected
   result and diagnose it safely. Stop only when it proves a lifecycle,
   authority, security, privacy, rollback, or project-identity boundary; correct
   recoverable in-scope validation failures under Standard Execution Authority.
...
5. Confirm no unresolved human-editing fields remain. Read and enforce the
   ...
   Builder-approved task receives Framework 2.0 Standard Execution Authority
   without another routine Owner gate.
...
9. Run proportional verification required by the active prompt and preserve
   reliable evidence until a relevant mutation invalidates it. Classify,
   diagnose, correct, and retest recoverable in-scope failures. Stop only on a
   genuine boundary or an Owner-reserved operation.
