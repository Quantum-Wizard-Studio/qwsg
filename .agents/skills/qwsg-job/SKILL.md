...
2. Run `bin/job --check` from the verified repository. Stop on every validation error.
...
5. Confirm no unresolved human-editing fields remain. Follow the prompt's Starting State Verification exactly.
...
9. Run every verification required by the active prompt, preserve rollback capability, and stop on material environmental differences.
